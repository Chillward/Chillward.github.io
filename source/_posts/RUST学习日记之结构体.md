...
如果整个实例是可变的，那我们就可以通过`user1.email = String::from("anotheremail@example.com");`来修改实例中字段的值。
...
**方法**（method）与函数类似：它们使用 `fn` 关键字和名称声明，可以拥有参数和返回值，同时包含在某处调用该方法时会执行的代码。不过方法与函数是不同的，因为它们在结构体的上下文中被定义（或者是枚举或 trait 对象的上下文，将分别在[第六章](https://kaisery.github.io/trpl-zh-cn/ch06-00-enums.html)和[第十八章](https://kaisery.github.io/trpl-zh-cn/ch18-02-trait-objects.html)讲解），并且它们第一个参数总是 `self`，它代表调用该方法的结构体实例。

### [定义方法](https://kaisery.github.io/trpl-zh-cn/ch05-03-method-syntax.html#定义方法)
...
### [带有更多参数的方法](https://kaisery.github.io/trpl-zh-cn/ch05-03-method-syntax.html#带有更多参数的方法)
...
### [关联函数](https://kaisery.github.io/trpl-zh-cn/ch05-03-method-syntax.html#关联函数)
...
要调用这个关联函数，我们使用结构体名和 `::` 语法；比如 `let sq = Rectangle::square(3);`。这个函数位于结构体的命名空间中：`::` 语法用于关联函数和模块创建的命名空间。[第七章](https://kaisery.github.io/trpl-zh-cn/ch07-02-defining-modules-to-control-scope-and-privacy.html)会讲到模块。

### [多个 `impl` 块](https://kaisery.github.io/trpl-zh-cn/ch05-03-method-syntax.html#多个-impl-块)
...
## [总结](https://kaisery.github.io/trpl-zh-cn/ch05-03-method-syntax.html#总结)
