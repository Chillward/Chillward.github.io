---
title: rt-Thread学习日记之时钟管理
date: 2025-08-02 07:09:46
tags: rt-thread
---
...
可以类比之前蓝桥杯的时候用到systick实现的"系统心跳"，不过rt-thread将其进行了进一步封装。在不同的硬件平台上，这个时钟节拍的实现方式是不一样的。

在stm32上，一般是由一个配置为中断模式的硬件定时器来实现的(并且一般是systick，当systick被占用时，可以由其他定时器代替)。

> 这个中断可以看做是系统心跳，中断之间的时间间隔取决于不同的应用，一般是 1ms–100ms，时钟节拍率越快，系统的实时响应越快，但是系统的额外开销就越大，从系统启动开始计数的时钟节拍数称为系统时间。RT-Thread 中，时钟节拍的长度可以根据 RT_TICK_PER_SECOND 的定义来调整，等于 1/RT_TICK_PER_SECOND 秒。

### [时钟节拍的实现方式](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=时钟节拍的实现方式)
...
上头这句话是什么意思，我去问了一下Ai，它给了我这样的回复

你提到的“系统硬件定时器链表”是一个关键概念，但根据RT-Thread的实现，它其实更准确地说是**软件定时器链表**，并且这个链表是由硬件定时器驱动的。

让我来为你详细解释一下这个机制。

### RT-Thread 的软件定时器系统

RT-Thread 并没有一个专门用于硬件定时器的链表。它只有一个**全局的软件定时器链表**，所有由 `rt_timer_create()` 创建的定时器，无论它们是单次定时还是周期性定时，都会被挂载到这个链表中。

这个链表是一个**按超时时间排序的链表**。系统在每次时钟节拍（Tick）中断中，都会检查这个链表。

### `rt_timer_check()` 函数的作用

`rt_timer_check()` 这个函数在每个时钟节拍中断中被调用，它的主要职责是：

1. **检查链表头部的定时器是否超时**：它会遍历软件定时器链表，比较每个定时器的超时时间与当前的系统时钟节拍 (`rt_tick`)。
2. **调用超时函数**：如果发现有定时器超时，它会调用你为该定时器设定的回调函数。
3. **移除定时器**：无论是单次定时器还是周期性定时器，在超时并执行完回调函数后，`rt_timer_check()` 都会将它从链表中移除。

### 周期性定时器的特殊处理

这里就是你提到的“周期性定时器会在它再次启动时被加入定时器链表”的机制。对于一个**周期性定时器**，当它超时并被移除后，系统会：

- **重新计算下一次超时时间**：根据它的周期值，计算出它下一次的超时节拍。
- **重新加入链表**：将它重新插入到软件定时器链表中的正确位置（按照新的超时时间排序）。

通过这种方式，周期性定时器才能不断地重复执行。

### 总结

所以，你所说的“系统硬件定时器链表”更准确的理解应该是：

一个**由硬件定时器中断驱动的、按超时时间排序的软件定时器链表**。硬件定时器提供周期性的“心跳”，而 `rt_timer_check()` 函数在每次“心跳”中检查这个链表，并处理那些超时的软件定时器。这个链表是 RT-Thread 软件定时器系统的核心，实现了所有定时器的管理、调度和执行。

### [获取时钟节拍](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=获取时钟节拍)
...
> 这个函数其实就类似于HAL库里头的HAL_GetTick();这个函数(函数名应该是这个)，不过rt-thread对其进行了进一步封装

## [定时器管理](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=定时器管理)
...
RT-Thread 的定时器提供了两类触发机制和两种执行模式。

#### 触发机制

- **单次触发定时器**：这类定时器在启动后只会触发一次事件，然后自动停止。
- **周期触发定时器**：这类定时器会周期性地触发事件，直到被手动停止。

### 执行模式

根据超时函数执行时的上下文环境，RT-Thread 的定时器可以分为 **HARD_TIMER** 和 **SOFT_TIMER** 两种模式。

#### HARD_TIMER 模式

HARD_TIMER 模式是 **RT-Thread 的默认定时器模式**。

- **执行上下文**：超时函数在**中断服务程序（ISR）**中执行。
- **优点**：响应速度快，实时性高。
- **缺点**：因为在中断中执行，超时函数的执行时间必须非常短。**不能**调用任何会导致当前上下文挂起的函数，例如 `rt_thread_delay()`、`rt_mutex_take()` 等。
- **使用场景**：适合执行一些简单、快速的操作，如设置一个标志位或发送一个信号量。

#### SOFT_TIMER 模式

SOFT_TIMER 模式需要通过宏定义 `RT_USING_TIMER_SOFT` 来启用。

- **执行上下文**：超时函数在一个专门的**定时器线程**中执行。
- **优点**：超时函数可以在独立的线程中执行，因此可以调用几乎所有 RT-Thread API，包括会引起阻塞或挂起的函数。
- **缺点**：响应时间不如 HARD_TIMER 模式快，因为它需要等待线程调度。
- **使用场景**：适合执行一些复杂、耗时较长的操作，或者需要调用阻塞型 API 的任务。
...
![image-20250802082923845](../images/image-20250802082923845.png)

### [定时器工作机制](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=定时器工作机制)
...
![定时器链表示意图](../images/05timer_linked_list.png)
...
![定时器链表插入示意图](../images/05timer_linked_list2.png)

> 这块讲的非常清晰明了

#### [定时器控制块](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=定时器控制块)
...
struct rt_timer
{
    struct rt_object parent; //一些rt-thread中的对象所具有的默认属性
...
> 这个其实直接理解成二叉树 + 链表就行了，只管用就完事了，算一种空间换时间

> 下面这块对于应用来说是比较重要的

### [定时器的管理方式](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=定时器的管理方式)
...
![定时器相关操作](../images/05timer_ops.png)

#### [创建和删除定时器](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=创建和删除定时器)
...
#### [初始化和脱离定时器](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=初始化和脱离定时器)
...
#### [启动和停止定时器](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=启动和停止定时器)
...
#### [控制定时器](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=控制定时器)
...
## [定时器应用示例](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=定时器应用示例)
...
## [高精度延时](https://www.rt-thread.org/document/site/#/rt-thread-version/rt-thread-standard/programming-manual/timer/timer?id=高精度延时)
...
> 这个其实就是我们之前在蓝桥杯嵌入式的时候利用systick手写延时的ProMax版本，这种写法可以码一下
